...
function searchMeal(e) {
  ...
  if (term.trim()) {
      ...
      .then(data => {
        ...
        if (data.meals === null) {
          ...
          mealsEl.innerHTML = data.meals
            ...
            .join(''); // to display this as a string
        }
      });
    // Clear search text
    search.value = '';
    ...
    if (!resultHeading.innerHTML == '') {
      ...
      resultHeading.innerHTML = '<p>Please fill in the search box</p>';
      setTimeout(() => {
...
function getMealById(mealID) {
  fetch(`https://www.themealdb.com/api/json/v1/1/lookup.php?i=${mealID}`)
    .then(res => res.json())
    .then(data => {
      const meal = data.meals[0]; // the api gives us an array
...
function addMealToDOM(meal) {
  ...
  // respecting the format of API
  // actually, there is no other way to work around
  for (let i = 1; i <= 20; i++) {
